...
- 6-step commutation with:
  ...
  - direction reversal
- Explicit commutation phase FSM with:
  - `PH_RUN`
  - `PH_OVERLAP`
  - `PH_DEADTIME`
...
| `0x1C` | `MAIN_WINDOW` | Transition-count window in 100 MHz motor-domain clocks |
...
Internally this is implemented as a small explicit phase FSM:

- `PH_RUN`: normal commutation state output pattern
...
- transition-phase timing on commutation changes
