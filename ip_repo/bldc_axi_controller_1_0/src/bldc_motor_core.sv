module bldc_motor_core (
    ...
    input  logic        brake_enable_reg,
    input  logic [11:0] duty_cmd_reg,
    input  logic [11:0] brake_duty_reg,
    ...
    typedef enum logic [1:0] {
        BR_OFF            = 2'd0,
        BR_ENTER_DEADTIME = 2'd1,
        BR_ACTIVE         = 2'd2,
        BR_EXIT_DEADTIME  = 2'd3
    } brake_phase_t;
    ...
    logic [15:0] brake_on_counts;
    logic [27:0] brake_product;
    logic [5:0]  drive_outputs_next;
    comm_phase_t comm_phase;
    brake_phase_t brake_phase;
    ...
    assign brake_active     = (brake_phase != BR_OFF);
    assign deadtime_active  =
        (comm_phase != PH_RUN) ||
        (brake_phase == BR_ENTER_DEADTIME) ||
        (brake_phase == BR_EXIT_DEADTIME);
    ...
    always @* begin
        brake_product = brake_duty_reg * pwm_period_reg;
        if (brake_duty_reg == 12'd0) begin
            brake_on_counts = 16'd0;
        end else if (brake_duty_reg == 12'hFFF) begin
            brake_on_counts = pwm_period_reg;
        end else begin
            brake_on_counts = brake_product[27:12];
        end
    end
    ...
    always @* begin
        ...
        if (brake_phase == BR_ACTIVE) begin
            if (pwm_counter < brake_on_counts) begin
                drive_outputs_next = 6'b01_01_01;
            end
        end else if (brake_phase != BR_OFF) begin
            drive_outputs_next = 6'b0;
        end else if (drive_enable_reg) begin
            case (comm_phase)
                PH_OVERLAP: begin
                    drive_outputs_next =
    ...
    always_ff @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if ((brake_phase == BR_OFF) && brake_enable_reg) begin
                active_comm_state <= requested_comm_state;
                comm_phase        <= PH_RUN;
                low_overlap_count <= 8'd0;
                deadtime_count    <= deadtime_reg;
                brake_phase       <= BR_ENTER_DEADTIME;
            end else if ((brake_phase == BR_EXIT_DEADTIME) && brake_enable_reg) begin
                deadtime_count <= deadtime_reg;
                brake_phase    <= BR_ENTER_DEADTIME;
            end else if (brake_phase != BR_OFF) begin
                case (brake_phase)
                    BR_ENTER_DEADTIME: begin
                        if (deadtime_count > 8'd1) begin
                            deadtime_count <= deadtime_count - 8'd1;
                        end else begin
                            deadtime_count <= 8'd0;
                            if (brake_enable_reg) begin
                                brake_phase <= BR_ACTIVE;
                            end else begin
                                brake_phase <= BR_OFF;
                            end
                        end
